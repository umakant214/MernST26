...
<Link className="nav-item active text-decoration-none" to="/admin-department"><svg className="ni" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" /></svg><span>Department Management</span></Link>
...
<div><div className="hdr-title">Department Management</div><div className="hdr-sub">Manage department portal users</div></div>
</header>
<main className="page-body flex-fill overflow-auto p-3">
<div className="page-intro"><h2>Department Management</h2><p>Overview of all active departments</p></div>
<div className="sc-card">
<div className="tbl-wrap"><table className="tbl"><thead><tr><th>Department</th><th>User Name</th><th>Email</th><th>Role</th></tr></thead><tbody>
{depts.length > 0 ? depts.map((d) => (
    <tr key={d._id}>
        <td><strong>{d.departmentName}</strong></td>
        <td>{d.name}</td>
        <td>{d.email}</td>
        <td><span className="sc-badge b-resolved">{d.role}</span></td>
    </tr>
)) : <tr><td colSpan="4" className="text-center p-4">No departments found.</td></tr>}
</tbody></table></div>
</div>
</main>
</div>
