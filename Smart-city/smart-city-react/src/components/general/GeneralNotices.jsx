    ...
    return (
        <>
            ...
            <nav className="gnav">
                ...
                <div className="d-flex align-items-center gap-1 ms-3 gnav-desktop-links"><Link className="gnav-link" to="/general-home">Home</Link><Link className="gnav-link" to="/general-about">About</Link><Link className="gnav-link" to="/general-guidelines">Guidelines</Link><Link className="gnav-link act" to="/general-notices">Notices</Link><Link className="gnav-link" to="/general-contact">Contact</Link></div>
                <div className="gnav-right gnav-desktop-links">
                    <span className="gnav-pill">{notices.length} active notices</span>
                ...
                <button aria-label="Open navigation" className="gnav-mob-toggle" id="gnav-mob-toggle"><svg fill="none" height="18" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" width="18"><line x1="3" x2="21" y1="6" y2="6" /><line x1="3" x2="21" y1="12" y2="12" /><line x1="3" x2="21" y1="18" y2="18" /></svg></button><div className="gnav-mob-menu" id="gnav-mob-menu"><Link className="gnav-link" to="/general-home">Home</Link><Link className="gnav-link" to="/general-about">About</Link><Link className="gnav-link" to="/general-guidelines">Guidelines</Link><Link className="gnav-link act" to="/general-notices">Notices</Link><Link className="gnav-link" to="/general-contact">Contact</Link><Link className="gnav-btn-ghost" to="/general-login">Login</Link><Link className="gnav-btn-solid" to="/general-register">Register Free</Link></div></nav>
            <div className="inner-hero">
                <div className="ih-tag">Notices</div>
                <h1 className="ih-title">Public Notices &amp; Announcements</h1>
            ...
            <section className="sec-wrap">
                <div className="sec-inner" style={{ 'maxWidth': '800px' }}>
                    {notices.length > 0 ? notices.map((notice, index) => (
                        <div className={index === 0 ? "notice-featured mb-3" : "notice-small mb-3"} key={notice._id}>
                            <div className={index === 0 ? "nf-tag" : "ns-tag info"}>{notice.category === 'Alert' ? '🔴 Alert' : '🆕 Notice'}</div>
                            <div className={index === 0 ? "nf-title" : "ns-title"}>{notice.title}</div>
                            <div className={index === 0 ? "nf-body" : "ns-body"}>{notice.content}</div>
                            <div className={index === 0 ? "nf-date" : "ns-date"}>🕐 {new Date(notice.createdAt).toDateString()} · Smart City Admin</div>
                        </div>
                    )) : (
                        <div className="text-center p-5 sc-card">
                            <h3>No active notices</h3>
                            <p className="text-muted">Stay tuned for more updates from the Municipal Corporation.</p>
                        </div>
                    )}
                </div>
            </section>
                ...
                <div className="container-xl">
                    ...
                    <div className="footer-bottom">
                        ...
                        <div className="d-flex gap-2">
                            ...
                            <div className="fsoc-btn"><svg fill="none" height="13" stroke="#9CA3AF" strokeWidth="2" viewBox="0 0 24 24" width="13"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37(0 00-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0020 4.77 5.07 5.07 0 0019.91 1S18.73.65 16 2.48a13.38 13.38 0 00-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 005 4.77a5.44 5.44 0 00-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 009 18.13V22" /></svg></div>
